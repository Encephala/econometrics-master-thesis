...
all_relevant_data <- column_to_rownames(all_relevant_data, var = "index")
...
```{r}
# Specific-to-general getting the model to have PD (implied) cov matrix
sem_model <- "mhi ~ 1  + cs104"
```

```{r}
sem_fit <- lavaan(sem_model, data = all_relevant_data, missing = "fiml.x")
