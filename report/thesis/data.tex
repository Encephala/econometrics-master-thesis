...
as for such individuals there is no within-person variability to be studied. However, under the assumption that there
are no individual-specific unmeasured confounders, the between-person variability would equal the within-person variability.
While this assumption is up for contention in the present study, as will be elaborated on in \cref{sec:methods:sem},
% TODO more precise reference?
it was chosen to leave such observation in the data for the purpose of maintaining statistical power.
...
Additionally, some variables are measured on an ordinal scale, but their effect on mental wellbeing is very unlikely to
