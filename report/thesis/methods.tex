...
The model in \cref{eq:methods:simple_regression} involves three variables (again, neglecting the mean component),
...
Firstly, if a model is just identified, each of these fit indices will by definition indicate a perfect fit, which is to
...
Rather, fit indices quantify fit in an absolute sense, where a poor fit reflects poor internal validity.
\Cref{sec:methods:cv} will discuss the approach used in this work for model selection.
...
\begin{equation}
    \text{TLI} = \frac{\chi^2 / df - \chi^2_b / df_b}{\chi^2 / df - 1}.
\end{equation}
While not strictly confined to the range of $[0, 1]$, the TLI is typically in that range. Again, higher is better,
and the cutoff for good fit is taken from \citeA{hu1999cutoff} as 0.95.
