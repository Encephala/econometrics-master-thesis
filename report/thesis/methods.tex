...
Just as $\beta$ in \cref{eq:modelling:mediation_example} cannot differentiate between the forward- and reverse effects,
...
being allowed to vary freely across time. As a result, if in reality there is AR behaviour to the mediators, the total
...
While the AR nature could be explicitly modelled, doing so would make modelling much more complex and error-prone,
so the fact that $\alpha$-significance is maintained justifies taking the reduced statistical power as a compromise.
